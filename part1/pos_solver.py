...
class Solver:
    ...
    def hmm_ve(self, sentence):
        sequence = []
        alpha_table = {i:{} for i in range(len(sentence))}
        beta_table = {i:{} for i in range(len(sentence))}
        for i in range(len(sentence)):
            word = sentence[i]
            if(i == 0):
                for pos in Solver.ALL_POS:
                    if(word in self.P_word_pos[pos]):
                        alpha_table[i][pos] = self.P_initial[pos] * self.P_word_pos[pos][word]
                    else:
                        alpha_table[i][pos] = self.P_initial[pos] * 0.0000001
            else:
                for cur_pos in Solver.ALL_POS:
                    if(word in self.P_word_pos[cur_pos]):
                        alpha_table[i][cur_pos] = self.P_word_pos[cur_pos][word]
                    else:
                        alpha_table[i][cur_pos] = 0.0000001
                    alpha_table[i][cur_pos] *= sum([self.P_transition[cur_pos][prev_pos] * alpha_table[i-1][prev_pos] for prev_pos in Solver.ALL_POS])
        ...
        for i in range(len(sentence)-1,-1,-1):
            if(i==len(sentence)-1):
                ...
                word = sentence[i+1]
                for cur_pos in Solver.ALL_POS:
                    beta_table[i][cur_pos] = 0
                    for next_pos in Solver.ALL_POS:
                        if(word in self.P_word_pos[next_pos]):
                            beta_table[i][cur_pos] += beta_table[i+1][next_pos] *\
                                                     self.P_transition[next_pos][cur_pos] *\
                                                     self.P_word_pos[next_pos][word]
                        else:
                            beta_table[i][cur_pos] += beta_table[i+1][next_pos] *\
                                                     self.P_transition[next_pos][cur_pos] *\
                                                     0.0000001
        ...
        for i in range(len(sentence)):
            prob_f = alpha_table[i]
            prob_b = beta_table[i]
            max_prob = 0
            max_pos = ''
            for pos in prob_f:
                cur_prob = prob_f[pos] * prob_b[pos]
                if(max_prob < cur_prob):
                    max_prob = cur_prob
                    max_pos = pos
            sequence.append(max_pos)
        return sequence
